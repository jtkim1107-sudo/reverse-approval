    ...
    sameHtml = `
      <div class="card" style="margin-top:8px;padding:10px;border:1px dashed var(--border)">
        <div style="display:flex;justify-content:space-between;align-items:center">
          ...
          ${same.eligible
            ? `<button class="btn sm" onclick="chooseAndSave1pltOption('${poId}','${poItemId}','same_product')">이 안으로 저장</button>`
  ...
  const mixedHtml = mixedList.map(m => `
    <div class="card" style="margin-top:8px;padding:10px;border:1px dashed var(--border)">
      <div style="display:flex;justify-content:space-between;align-items:center">
        ...
        <button class="btn sm" onclick="chooseAndSave1pltOption('${poId}','${poItemId}','mixed_load:${esc(m.product_id)}')">이 안으로 저장</button>
      </div>
...
async function loadPOLoadFillProposal(poId, items) {
  ...
  for (const it of items) {
    ...
    // canonical PLT>=2(사람이 고를 선택지가 없음) - GET 결과를 그리지 않고
    // 곧바로 /save를 호출해서 *** 그 응답(saved.proposal)만 *** 화면에 그림.
    await _saveAndRenderLoadFillProposal(poId, it.id, el, null);
  }
}
...
async function loadPOBatchLoadFillProposal(poId) {
  ...
  // *** GET 결과(preview)는 여기서 "저장 가능한지" 판단에만 씀 - 실제로
  // 저장 가능하면(batch_plan.applicable) 곧바로 /save를 호출해서 그 응답만
  // 화면에 그림(요구사항 4: 화면-DB 괴리 방지). 저장 불가(미배정 상품 있음)
  // 하면 preview를 참고용으로만 보여주고 저장 버튼은 안 보여줌(요구사항 6).
  if (!preview.batch_plan || !preview.batch_plan.applicable) {
    el.innerHTML = _batchLoadFillProposalCardHtml(poId, preview.batch_plan, null);
    return;
  }

  try {
    const saveResp = await fetch(`${LIVE_STOCK_API_BASE}/api/purchase-orders/load-fill-proposal/save`,
      { method: "POST", headers: { Authorization: `Bearer ${jwt}`, "Content-Type": "application/json" },
        body: JSON.stringify({ purchase_order_id: poId }) });
    if (!saveResp.ok) {
      const errBody = await saveResp.json().catch(() => ({}));
      el.innerHTML = _batchLoadFillProposalCardHtml(poId, preview.batch_plan, null);
      return toast(errBody.detail || "batch 제안 저장에 실패했습니다.");
    }
    const saved = await saveResp.json();
    _batchLoadFillProposalRuns[poId] = { run_id: saved.run_id, proposal_hash: saved.proposal_hash };
    if (!document.getElementById("po-batch-load-fill-proposal")) return;
    // *** saved.proposal.batch_plan이 화면 렌더링의 유일한 원본(요구사항 4) ***
    el.innerHTML = _batchLoadFillProposalCardHtml(poId, saved.proposal?.batch_plan, saved);
  } catch (e) {
    toast("batch 제안 저장 요청에 실패했습니다.");
  }
}
