...
/* ---------- 구글 캘린더로 일정 공유 ----------
   앱이 캘린더에 자동으로 손대는 일은 없습니다. 사람이 📅 버튼을 눌렀을 때만,
   구글 캘린더 '일정 만들기' 화면이 날짜·제목이 채워진 채로 새 탭에 열립니다.
   저장할지 말지, 누구와 공유할지는 구글 캘린더에서 본인이 정합니다. */

const gcalDay = d => String(d).replace(/-/g, "");  // 2026-09-01 → 20260901
...
function gcalUrl(ev) {
  const q = new URLSearchParams({
    action: "TEMPLATE",
    text: ev.title,
    dates: `${gcalDay(ev.date)}/${gcalDay(addDaysStr(ev.date, 1))}`,
    details: ev.detail || "",
    ctz: "Asia/Seoul",
  });
...
async function viewSettings() {
  ...
  return `
    ...
    <div class="card">
      <h2>📅 구글 캘린더로 일정 공유</h2>
      <p style="color:var(--text-sub);font-size:13.5px;margin-bottom:12px">
        업무 지시·자금일보 목록의 <b>📅</b> 버튼을 누르면, 구글 캘린더 <b>일정 만들기</b> 화면이
        날짜와 내용이 채워진 채로 새 탭에 열립니다. 저장할지, 누구를 초대할지는 거기서 정하시면 됩니다.</p>
      <p style="color:var(--text-sub);font-size:13px">
        ※ 앱이 캘린더를 몰래 들여다보거나 자동으로 일정을 넣지 않습니다 — 버튼을 누를 때만 동작합니다.</p>
    </div>
